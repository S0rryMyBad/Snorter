#!/bin/bash
# lazy script for installing Snort , Barnyard2 , Pulledpork
# Tested on: Kali linux , Ubuntu 14.04
# Copyright (c) 2017 - betmenwasdie // betmenwasdie@gmail.com
# Usage: $ sudo ./snorter.sh -o OINKCODE -i INTERFACES
...
function check_root() {
	if [ "$(id -u)" != 0 ]; then
		echo -ne "\n\t\t${RED}[-] ERROR:${NOCOLOR} Run this script as ${BOLD}ROOT${NOCOLOR}\n"
		exit 1
	fi
}
